...
if __name__=='__main__':
    ...
    if words[0] == 'D' :
        ...
        print "computing now ... usually takes 3hours when running usa.txt "
        
        path = []
        dist, p = bellman_ford(bellgraph, srcNode)
        cost = dist[dstNode]
        ...
        print "The distance from node %d to node %d using Bellman-Ford algorithm is %d"  %(srcNode,dstNode,cost)
        print "The execution time take %d milliseconds." %taketime
        print "The shortest path from node %d to node %d is %s." %(srcNode,dstNode,path)  
